from NadiasBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from NadiasBot import DRAGONS
...
async def saitama_is_admin(chat_id: int):
    status = False
    NadiasBot = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if NadiasBot.id == user.id:
            status = True
            break
    return status
